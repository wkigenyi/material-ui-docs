...
>(function StoreTemplateImage({ brand, ...props }, ref) {
  return (
    <Image
      ref={ref}
      src={`/static/branding/bankayo/products/loanproduct/loan-${
      ...
      sx={(theme) =>
        theme.applyDarkStyles({
          content: `url(/static/branding/bankayo/products/loanproduct/loan-${
            Object.keys(linkMapping).indexOf(brand) + 1
          }-dark.png)`,
        })
      }
...
export function PrefetchLoanProductImages() {
  function makeImg(mode: string, num: number) {
    return {
      loading: 'lazy' as const,
      width: '900',
      height: '494',
      src: `/static/branding/bankayo/products/loanproduct/loan-${num}-${mode}.png`,
    };
  }
  return (
    <Box
...
export default function LoanProductShowCasing() {
  return (
    <Box
      sx={{
        mx: { xs: -2, sm: -3, md: 0 },
