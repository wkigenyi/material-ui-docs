...
>(function StoreTemplateImage({ brand, ...props }, ref) {
  return (
    <Image
      ref={ref}
      src={`/static/branding/bankayo/products/reports/reports-${
      ...
      sx={(theme) =>
        theme.applyDarkStyles({
          content: `url(/static/branding/bankayo/products/reports/reports-${
            Object.keys(linkMapping).indexOf(brand) + 1
          }-dark.png)`,
        })
      }
...
export function PrefetchReportingImages() {
  function makeImg(mode: string, num: number) {
    return {
      loading: 'lazy' as const,
      width: '900',
      height: '494',
      src: `/static/branding/bankayo/products/reports/reports-${num}-${mode}.png`,
    };
  }
  return (
    <Box
...
export default function ReportShowCasing() {
  return (
    <Box
      sx={{
        mx: { xs: -2, sm: -3, md: 0 },
