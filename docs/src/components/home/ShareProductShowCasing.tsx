...
>(function StoreTemplateImage({ brand, ...props }, ref) {
  return (
    <Image
      ref={ref}
      src={`/static/branding/bankayo/products/shareproduct/shares-${
      ...
      sx={(theme) =>
        theme.applyDarkStyles({
          content: `url(/static/branding/bankayo/products/shareproduct/shares-${
            Object.keys(linkMapping).indexOf(brand) + 1
          }-dark.png)`,
        })
      }
...
export function PrefetchShareProductImages() {
  function makeImg(mode: string, num: number) {
    return {
      loading: 'lazy' as const,
      width: '900',
      height: '494',
      src: `/static/branding/bankayo/products/shareproduct/shares-${num}-${mode}.png`,
    };
  }
  return (
    <Box
...
export default function ShareProductShowCasing() {
  return (
    <Box
      sx={{
        mx: { xs: -2, sm: -3, md: 0 },
